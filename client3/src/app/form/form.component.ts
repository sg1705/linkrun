...
export class FormComponent implements OnInit {
  ...
  @ViewChild("link", { static: true }) inputLink: ElementRef;
  @ViewChild("url", { static: true }) inputUrl: ElementRef;
  @ViewChild("linkList", { static: true }) linkList: LinkListComponent;
  ...
  private initialize() {
    if (this.router.url.indexOf("/link/edit") > -1) {
      ...
      // get the link
      this.linkService.getLink(this.linkId).then(link => {
        ...
        this.linkFormGroup.controls["acl"].setValue(
          link.acl != Link.LINK_ACL_DEFAULT
        );
      });
  ...
  onSubmit(l): Promise<boolean> {
    if (this.mode == "edit") {
      return new Promise((resolve, reject) => {
        ...
        this.linkService
          .updateLink(new Link(this.linkId, link, url, description, acl))
          .then(link => {
      ...
      return new Promise((resolve, reject) => {
        this.linkService
          .createLink(
            new Link(0, l.link, l.url, l.description, this.getACL(l.acl))
          )
          .then(link => {
  ...
  private showConfirmationDialog(resolve, message, link: Link) {
    ...
    if (this.linkAclFeature && this.aclMessage) {
      publicLink = this.helperService.generatePublicLinkFromOrgShortName(
        link.link,
        this.orgShortName
      );
    }
