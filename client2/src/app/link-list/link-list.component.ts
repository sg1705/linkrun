...
export class LinkListComponent implements OnInit {
  ...
  copyLink(link:string) {
    this.clipboardService.copyFromContent(this.helperService.generateFullShortLink(link));
    this.openSnackBar('ShortLink copied');
  }

  copyPublicLink(link:string) {
    this.clipboardService.copyFromContent(this.helperService.generatePublicLink(link, this.userObject));
...
export class LinkDataSource extends DataSource<any> {
  ...
  connect(): Observable<Link[]> {
    ...
    return from(this.linkService.getLinks().then(links => {
